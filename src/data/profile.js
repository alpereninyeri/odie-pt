export const profile = {
  ...
  stats: [
    {
      key: 'str', label: 'KUV', name: 'Kuvvet', val: 80,
      ...
      detail: [
        { label: 'İtiş', val: 'A+' }, { label: 'Çekiş', val: 'B+' },
        { label: 'Patlayıcı', val: 'A' }, { label: 'Tepe yük', val: '65kg 🆕' },
      ],
    },
    {
      key: 'agi', label: 'ÇEV', name: 'Çeviklik', val: 77,
      color: 'var(--blu)', icon: '💨',
      ...
      detail: [
        { label: 'Hava hissi', val: 'A' }, { label: 'Zemin hızı', val: 'B+' },
        { label: 'Reaksiyon', val: 'B+' }, { label: 'Yön değişimi', val: 'B-' },
      ],
    },
    {
      key: 'end', label: 'DAY', name: 'Dayanıklılık', val: 73,
      color: 'var(--grn)', icon: '🫀',
      ...
      detail: [
        { label: 'Kardiyo tabanı', val: 'B' }, { label: 'Günlük adım', val: '12k+' },
        { label: 'Toparlanma', val: 'B' }, { label: 'İş kapasitesi', val: 'B+' },
      ],
    },
    {
      key: 'dex', label: 'BEC', name: 'Koordinasyon', val: 68,
      color: 'var(--pur)', icon: '🎯',
      desc: 'Parkour alt-üst vücut entegrasyonunu doğrudan çalıştırdı. Yerde koordinasyon belirgin gelişti.',
      coach: 'Parkour tam beceri işi — iniş mekanikleri, yön değiştirme, zemin hissi. Alt vücut koordinasyonu C+\'tan B-\'ye çıktı. Bacak gücü gelince B+ garantili.',
      detail: [
        { label: 'Üst gövde', val: 'A-' }, { label: 'Alt gövde', val: 'B-' },
        { label: 'Denge', val: 'B' }, { label: 'Zamanlama', val: 'B+' },
      ],
    },
    {
      key: 'con', label: 'GÖV', name: 'Core (Kritik)', val: 18,
      color: 'var(--org)', icon: '🟠', critical: false,
      desc: 'İLK KEZ izole core yapıldı — Hollow Rock + HLR + Çakı. 7 set. Core motoru ateşlendi.',
      coach: '14 Nis\'te ilk gerçek core bloğu: Hollow Rock 3×20sn, HLR 2×8, Çakı 2×12. Bu istatistik artık hareket ediyor. Tutarlı devam ederse GÖV 30\'u geçer ve Back Flip kilidi açılır.',
      detail: [
        { label: 'Hollow Body', val: 'D' }, { label: 'L-Sit', val: 'F' },
        { label: 'HLR', val: 'D+' }, { label: 'Rotasyon kilidi', val: 'F' },
      ],
    },
    {
      key: 'sta', label: 'STM', name: 'Stamina', val: 63,
      ...
      detail: [
        { label: 'Seans ort.', val: '64dk' }, { label: 'En uzun', val: '150dk 🆕' },
        { label: 'Dinlenme', val: 'Orta' }, { label: 'Yakıt verimi', val: 'B-' },
      ],
    },
  ],
  ...
  skills: [
    {
      ...
      items: [
        { status: 'done', name: 'Front Flip',   desc: 'Patlayıcı güç + hava hissi',        val: 'Açıldı', valColor: 'var(--grn)' },
        { status: 'done', name: 'Dive Roll',    desc: 'Yumuşak iniş tekniği',              val: 'Açıldı', valColor: 'var(--grn)' },
        { status: 'done', name: 'Round Off',    desc: 'Akrobasi geçiş hareketi',           val: 'Açıldı', valColor: 'var(--grn)' },
        { status: 'prog', name: 'Barani',       desc: 'Front flip + yarım dönüş denemesi', val: 'Yolda' },
        { status: 'lock', name: 'Back Flip',    desc: 'Gövde kontrolü olmadan risk!',      val: 'Kilitli', req: 'Şart: gövde rütbesi C' },
        { status: 'lock', name: 'Full Twist',   desc: 'Front flip + tam dönüş',            val: 'Kilitli', req: 'Şart: Barani açık' },
      ],
    },
    {
      branch: '⚔️ STRENGTH TREE',
      items: [
        { status: 'done', name: 'Dead Hang Elite',      desc: '1:20+ — güçlü tutuş',             val: 'Açıldı', valColor: 'var(--grn)' },
        { status: 'done', name: 'Muscle-Up',            desc: '1\'den 3 tekrara çıkıldı.',       val: 'Açıldı', valColor: 'var(--grn)' },
        { status: 'done', name: 'Bench Press 65kg',     desc: '65kg PR — 14 Nis 2026 kırıldı.',  val: 'Açıldı', valColor: 'var(--grn)' },
        { status: 'lock', name: 'Muscle-Up ×5 Clean',   desc: '5 temiz tekrar stabilize et',     val: 'Kilitli', req: 'Şart: 3 tekrar stabil' },
        { status: 'lock', name: 'One-Arm Hang',         desc: 'Tek kol asılı kalma',             val: 'Kilitli', req: 'Şart: KUV 85' },
      ],
    },
    {
      branch: '🧘 MOBILITY TREE',
      items: [
        { status: 'done', name: 'Hip Flexor Base',      desc: 'Flip için yeterli esneklik',      val: 'Açıldı', valColor: 'var(--grn)' },
        { status: 'prog', name: 'Shoulder Flexibility', desc: 'Aktif çalışılıyor',               val: 'Yolda' },
        { status: 'lock', name: 'Active Splits',        desc: 'Akrobasi kapsamını genişletir',   val: 'Kilitli', req: 'Şart: 3 ay günlük mobilite' },
        { status: 'lock', name: 'Bridge',               desc: 'Omurga esnekliği',                val: 'Kilitli', req: 'Şart: gövde rütbesi B' },
      ],
    },
    {
      branch: '🔥 CORE TREE', warning: true,
      items: [
        { status: 'prog', name: 'Hollow Body 30sn',  desc: '14 Nis: 3×20sn ✓ — 30sn\'ye ulaş',          val: 'Yolda' },
        { status: 'lock', name: 'L-Sit 10sn',        desc: 'Statik core + hip flexor — başlamak için hazır', val: 'Kilitli', req: 'Şart: GÖV E' },
        { status: 'lock', name: 'Plank Variations',  desc: 'Lateral stabilizasyon',                    val: 'Kilitli', req: 'Şart: gövde rütbesi D' },
        { status: 'lock', name: 'Dragon Flag',       desc: 'İleri seviye core kontrol',                val: 'Kilitli', req: 'Şart: gövde rütbesi C' },
        { status: 'lock', name: 'Front Lever Tuck',  desc: 'Üst vücut + core entegrasyonu',            val: 'Kilitli', req: 'Şart: gövde rütbesi B' },
      ],
    },
  ],
  ...
  health: {
    metrics: [
      ...
      { icon: '🛡️', label: 'Hazırlık',       val: '85/100',   sub: 'sağlam',                  color: 'var(--grn)' },
  ...
  achievements: [
    { icon: '💀', name: 'İlk Kan',          desc: 'İlk antrenman',         unlocked: true,  date: 'Ock 2025' },
    { icon: '🔟', name: 'Onlu Kulüp',       desc: '10 seans tamamlandı',   unlocked: true,  date: 'Şub 2025' },
    { icon: '💫', name: 'Yerçekimi Kıran',  desc: 'İlk Muscle-Up',         unlocked: true,  date: 'Mar 2025' },
    { icon: '🌀', name: 'Hava Doğuşu',      desc: 'İlk Front Flip',        unlocked: true,  date: 'Mar 2025' },
    { icon: '🤲', name: 'Demir Tutuş',      desc: 'Dead Hang 1dk+',        unlocked: true,  date: 'Nis 2025' },
    ...
    { icon: '🔒', name: 'Altın Koşu',       desc: 'Altın rütbeye çık',     unlocked: false, req: '+760 XP' },
  ],
