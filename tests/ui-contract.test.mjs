...
test('body and session screens expose compact tappable detail surfaces', () => {
  ...
  assert.doesNotMatch(main, /renderAccessPrompt|data-access|access-sheet|window\.prompt/)
...
test('browser data path is a public read-only direct Hevy snapshot without password headers', () => {
  ...
  assert.doesNotMatch(snapshotApi, /authorizeAppRequest|appAuthConfigured|ODIE_APP_ACCESS_TOKEN/)
  ...
  assert.doesNotMatch(dashboardStore, /appHeaders|hasAppAccessToken|Authorization/)
  assert.match(dashboardStore, /cache: 'no-store'/)
