...
async function hevyFetch(path, { retry = 2, timeoutMs = REQUEST_TIMEOUT_MS } = {}) {
  let lastErr
  for (let attempt = 0; attempt <= retry; attempt++) {
    ...
    let response
    try {
      response = await fetch(`${HEVY_BASE}${path}`, {
        headers: hevyHeaders(),
        signal: controller.signal,
      })
    } catch (error) {
      lastErr = error?.name === 'AbortError'
        ? new Error(`Hevy timeout on ${path}`)
        : error
      if (attempt < retry) await sleep((attempt + 1) * 750)
      continue
    } finally {
    ...
    if (response.status === 429 || response.status >= 500) {
      ...
      if (attempt < retry) await sleep((attempt + 1) * 1000)
      continue
    }
    if (!response.ok) {
...
export async function listExerciseTemplates(page = 1, pageSize = MAX_TEMPLATE_PAGE_SIZE) {
  const size = Math.max(1, Math.min(pageSize, MAX_TEMPLATE_PAGE_SIZE))
  return hevyFetch(`/v1/exercise_templates?page=${page}&pageSize=${size}`)
}
