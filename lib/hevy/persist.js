...
async function persistAthleteMemory({ profileId, workoutId, session, nextStats, nextClass, survival }) {
...
export async function ingestNormalizedExternalWorkout(payload, { onUpdate = 'replace', generateCoach = true } = {}) {
  ...
  if (workoutId) {
    ...
    await persistAthleteMemory({ profileId: profile.id, workoutId, session, nextStats, nextClass, survival })
  }
