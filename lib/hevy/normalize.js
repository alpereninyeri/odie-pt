...
const CORE_KEYWORDS = [
  ...
  'sit up', 'situp', 'hollow', 'core', 'ab wheel', 'mountain climber',
]
...
function detectHevyOntologySignals(input = '') {
  return detectOntologySignalsBounded(input)
}
...
function impactTagsFromSignals(signals = []) {
  const rows = Array.isArray(signals) ? signals : []
  const maxMatchedLength = Math.max(0, ...rows.map(signal => Number(signal.matchedLength) || 0))
  const strongestNonLocomotion = Math.max(
    0,
    ...rows
      .filter(signal => signal.blockKind !== 'locomotion')
      .map(signal => Number(signal.matchedLength) || 0),
  )
  const relevantSignals = rows.filter(signal => {
    const matchedLength = Number(signal.matchedLength) || 0
    if (signal.id === 'terrain') return true
    if (maxMatchedLength && matchedLength < maxMatchedLength * 0.55) return false
    if (
      signal.blockKind === 'locomotion'
      && ['walking', 'running'].includes(signal.id)
      && strongestNonLocomotion > matchedLength
    ) {
      return false
    }
    return true
  })
  const tags = new Set(deriveTagsFromSignals(relevantSignals))
  if (relevantSignals.some(signal => signal.typeHint === 'Calisthenics')) {
    tags.add('body-control')
  }
  return [...tags].slice(0, 12)
}

function templateFromMap(templateById, id) {
  if (!id || !templateById) return null
  if (typeof templateById.get === 'function') return templateById.get(String(id)) || templateById.get(id) || null
  return templateById[String(id)] || null
}
...
export function muscleTargetsFromHevyTemplate(template = {}) {
  const primary = normalizeMuscleGroup(template.primary_muscle_group ?? template.primaryMuscleGroup)
  const secondary = template.secondary_muscle_groups ?? template.secondaryMuscleGroups
  const rows = [
    primary ? { muscle: primary, role: 'primary' } : null,
    ...(Array.isArray(secondary) ? secondary : [])
      .map(muscle => ({ muscle: normalizeMuscleGroup(muscle), role: 'secondary' })),
  ].filter(Boolean)
  const targets = new Map()

  for (const row of rows) {
    const regionId = HEVY_MUSCLE_REGION_MAP[row.muscle]
    if (!regionId) continue
    const current = targets.get(regionId)
    if (!current || row.role === 'primary') {
      targets.set(regionId, {
        regionId,
        role: row.role,
        source: 'hevy-template',
      })
    }
  }
...
function toOdieExercises(hevyExercises = [], ontologyRows = [], templateById = new Map()) {
  return (hevyExercises || []).map((ex, index) => {
    const exerciseTemplateId = String(ex.exercise_template_id || ex.exerciseTemplateId || '').trim()
    const template = templateFromMap(templateById, exerciseTemplateId)
    return {
      name: ex.title || '(?)',
      exerciseTemplateId: exerciseTemplateId || null,
      muscleTargets: muscleTargetsFromHevyTemplate(template || {}),
      impactTags: impactTagsFromSignals(ontologyRows[index]?.signals || detectHevyOntologySignals(ex.title || '')),
...
export function normalizeHevyWorkout(workout, { templateById = new Map() } = {}) {
  if (!workout || !workout.id) {
    throw new Error('normalizeHevyWorkout: workout veya id eksik')
  }
  const ontology = collectOntologySignals(workout)
  const odieExercises = toOdieExercises(workout.exercises || [], ontology.exerciseRows, templateById)
